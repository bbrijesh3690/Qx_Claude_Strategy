...
export function checkSeries(s, opts = {}) {
  ...
  // Splice seams: an adjacent-minute move far outside this series' own
  // typical move. Real 1m FX does not jump 30% in a minute; two different
  // instruments stitched together do.
  const med = medianAbsLogReturn(c);
  const limit = Math.max(jumpFloor, med * jumpMultiple);
  const jumps = [];
  for (let i = 1; i < c.length; i++) {
    ...
    if (Math.max(seam, body) > limit) jumps.push({ time: c[i].time, move: Number(Math.max(seam, body).toFixed(5)) });
  }
  if (jumps.length) add(FAIL, "impossible_jump", `${jumps.length} adjacent-minute moves > ${(limit * 100).toFixed(2)}% (first at ${new Date(jumps[0].time).toISOString()})`);
  ...
  return {
    ...
    medianAbsReturn: med, jumps: jumps.slice(0, 5),
